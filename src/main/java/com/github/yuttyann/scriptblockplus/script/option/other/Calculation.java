...
public class Calculation extends BaseOption {
	...
	static {
		VALUES.add(new CalculationValue() {

			@SuppressWarnings("deprecation")
			@Override
			public double getValue(BaseOption baseOption, String name) throws Exception {
				if (name.startsWith("%players_in_range_") && name.charAt(name.length() - 1) == '%') {
					name = name.substring(0, name.length() - 1);
					String distance = StringUtils.split(name, "%players_in_range_")[1];
					int i = 10;
					try {
				...
				if (name.startsWith("%player_ping_") && name.charAt(name.length() - 1) == '%') {
					name = name.substring(0, name.length() - 1);
					Player target = Bukkit.getPlayer(StringUtils.split(name, "%player_ping_")[1]);
				...
				if (name.startsWith("%objective_score_") && name.charAt(name.length() - 1) == '%') {
					name = name.substring(0, name.length() - 1);
					ScoreboardManager manager = Bukkit.getScoreboardManager();
					Scoreboard board = manager.getMainScoreboard();
					return (int) board.getObjective(name).getScore(player).getScore();
				}
				switch (name) {
				case "%server_online%":
