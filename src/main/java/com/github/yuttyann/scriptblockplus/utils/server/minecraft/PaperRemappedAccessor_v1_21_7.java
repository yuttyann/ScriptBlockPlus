...
/**
 * Corelate-Bukkit PaperRemappedAccessor for Minecraft 1.21.7+
 * @author yuttyann44581
 */
public class PaperRemappedAccessor_v1_21_7 implements NativeAccessor {
    ...
    PaperRemappedAccessor_v1_21_7() throws ReflectiveOperationException {
        // sendPacket
        ...
        // getComponentFromJson - for 1.21.7+, try different approaches
        try {
            // First try with HolderLookup$Provider parameter
            method(NETWORK_CHAT.getClass("Component"))
                .modifiers(PUBLIC, STATIC)
                .name("literal")
            ...
            try {
                // Try Component.Serializer inner class
                Class<?>[] innerClasses = NETWORK_CHAT.getClass("Component").getDeclaredClasses();
                Class<?> serializerClass = null;
                for (Class<?> innerClass : innerClasses) {
                    if (innerClass.getSimpleName().equals("Serializer")) {
                        serializerClass = innerClass;
                        break;
                    }
                }
                if (serializerClass != null) {
                    method(serializerClass)
                        .modifiers(PUBLIC, STATIC)
                        .name("fromJson")
                        .returnType(NETWORK_CHAT.getClass("MutableComponent"))
                    ...
                    throw new Exception("Serializer class not found");
                }
            } catch (Exception e2) {
                try {
                    // Fallback: try ComponentSerialization if it exists
                    method(NETWORK_CHAT.getClass("ComponentSerialization"))
                        .modifiers(PUBLIC, STATIC)
                        .name("fromJson")
                        .returnType(NETWORK_CHAT.getClass("MutableComponent"))
                        .parameterTypes(String.class, CORE.getClass("HolderLookup$Provider"))
                    ...
                    // Final fallback: try simple fromJson method
                    method(NETWORK_CHAT.getClass("Component"))
                        .modifiers(PUBLIC, STATIC)
                        .name("Serializer$fromJson")
                        .returnType(NETWORK_CHAT.getClass("MutableComponent"))
                        .parameterTypes(String.class)
                        .findFirst("Serializer.fromJson");
                }
        ...
        // getComponentToJson - for 1.21.7+, try different approaches
        try {
            // Try Component.Serializer inner class first
            Class<?>[] innerClasses = NETWORK_CHAT.getClass("Component").getDeclaredClasses();
            Class<?> serializerClass = null;
            for (Class<?> innerClass : innerClasses) {
                if (innerClass.getSimpleName().equals("Serializer")) {
            ...
            try {
                ...
                try {
                    // Another fallback: try Component with single parameter
                    method(NETWORK_CHAT.getClass("Component"))
                        .modifiers(PUBLIC, STATIC)
                        .name("Serializer$toJson")
                        .returnType(String.class)
                        .parameterTypes(NETWORK_CHAT.getClass("Component"))
                        .findFirst("Serializer.toJson");
                } catch (Exception e3) {
                    // Final fallback: use toString method as alternative
                    method(NETWORK_CHAT.getClass("Component"))
                        .modifiers(PUBLIC, -STATIC)
                        .name("getString")
                        .returnType(String.class)
    ...
    public Object getComponentFromJson(@NotNull String json) throws ReflectiveOperationException {
        try {
            // Try with HolderLookup$Provider parameter first
            return m.invokeStatic("Serializer.fromJson", json, m.invokeStatic("CraftRegistry.getMinecraftRegistry"));
        } catch (Exception e) {
            try {
                // Try without HolderLookup parameter
                return m.invokeStatic("Serializer.fromJson", json);
            } catch (Exception e2) {
                // Final fallback: try Component.literal for simple text
                return NETWORK_CHAT.getClass("Component").getDeclaredMethod("literal", String.class).invoke(null, json);
            }
        }
    }
    ...
    public String getComponentToJson(@NotNull Object component) throws ReflectiveOperationException {
        try {
            ...
            try {
                // Try without HolderLookup parameter
                return (String) m.invokeStatic("Serializer.toJson", component);
            } catch (Exception e2) {
                try {
                    // Try getString method
                    return (String) component.getClass().getDeclaredMethod("getString").invoke(component);
                } catch (Exception e3) {
